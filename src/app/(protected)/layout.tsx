...
const actionItems = [
  {
    ...
    icon: (
      <svg
        ...
        strokeLinecap="round"
        strokeLinejoin="round"
        className="lucide lucide-heart w-6 h-6 text-[#E6B312]"
      >
  ...
  },
  {
    path: "/profile",
    label: "profile",
    icon: null, // Custom rendering for profile
...
const ClientLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const changeLanguage = (selectedOption: any) => {
    ...
    setSelectedLang(languages.find((lang) => lang.code === selectedOption.value) || languages[0])
  }
  ...
  const options = languages.map((lang) => ({
    ...
    label: (
      <div className="flex items-center">
        <img src={lang.flag || "/placeholder.svg"} alt={lang.label} width={24} height={16} className="mr-2" />
        {lang.label}
      </div>
    ),
  }))
  ...
  return (
    <div className="min-w-[320px] w-full">
      <header className="fixed top-0 w-full bg-[oldlace] dark:bg-black z-10 shadow-md">
        <div className="w-full p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <h1>
                <span className="text-2xl md:text-3xl text-[#E6B312]">Ҳунари</span>
                <span className="text-2xl md:text-3xl text-[#27A49C]"> ман</span>
              </h1>
            </div>
            <button
          ...
          <div className="flex flex-col md:flex-row gap-4 md:items-center w-full md:w-auto">
            <div className="flex items-center border w-full md:w-[300px] pr-3 gap-2 bg-white border-[#E6B312] h-[46px] rounded-[5px] overflow-hidden">
              <input
                ...
                type="text"
              />
              <svg fill="#6B7280" viewBox="0 0 30 30" height="22" width="22" xmlns="http://www.w3.org/2000/svg">
              ...
              <svg fill="#6B7280" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 32 32" width="20px">
                <g fill="#6B7280">
            ...
            <div className="flex justify-between items-center gap-2">
              <Select
                ...
                value={{
                  ...
                  label: options.find((opt) => opt.value === selectedLang.code)?.label,
                }}
        ...
        <nav
          className={`w-full transition-all duration-300 ${isMenuOpen ? "max-h-[500px]" : "max-h-0 md:max-h-[500px]"} md:block overflow-hidden`}
        >
              ...
              {actionItems.map((item, index) =>
                item.path !== "/profile" ? (
                  <button
                    key={item.path}
                    onClick={() => navigateTo(item.path)}
                    className={`flex items-center gap-1 transition-colors ${item.hoverColor} ${isActive(item.path) ? item.activeColor : ""}`}
                  >
                    {item.icon}
                  </button>
                ) : (
                  <button
                    key={item.path}
                    onClick={() => navigateTo(item.path)}
                    className={`flex items-center gap-2 p-2 rounded-full transition-colors ${item.hoverColor} ${isActive(item.path) ? item.activeColor : ""}`}
                  >
                    <div className="relative w-8 h-8 rounded-full overflow-hidden">
                      {user.imageUsers ? (
                        <Image alt="User" fill src={user.imageUsers || "/placeholder.svg"} className="object-cover" />
                      ) : (
      ...
      <main className="mt-[200px] md:mt-[180px] min-h-screen bg-[oldlace] dark:bg-black px-4">{children}</main>
    </div>
  )
}
...
const NavItem = ({
  ...
  return (
    ...
    >
      <span className={isActive ? "text-[#E6B312] dark:text-[#27A49C]" : "text-gray-700 dark:text-gray-300"}>
        {icon}
      </span>
      <span className="text-base">{label}</span>
      {isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-[#E6B312] dark:bg-[#27A49C]"></span>}
    </button>
  )
}
